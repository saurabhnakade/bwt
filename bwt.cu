...
__device__ bool compare_rotations(const int& a, const int& b, char* inputMessage, int n) {
    if (a < 0) {
        return false;
    }
    if (b < 0) {
    ...
    for(size_t i = 0; i < n; i++) {
        if (inputMessage[(a + i) % n] != inputMessage[(b + i) % n]) {
            return inputMessage[(a + i) % n] < inputMessage[(b + i) % n];
        }
    }
    return false;
}
...
__global__ void bitonic_sort_step(int* table, int table_size, int j, int k, char* inputMessage, int n) {
    unsigned int i = threadIdx.x + blockDim.x * blockIdx.x;
    unsigned int ixj = i ^ j;
    if(i < table_size) {
        if(ixj > i) {
            if ((i & k) == 0) {
                if (compare_rotations(table[ixj], table[i], inputMessage, n)) {
                    int temp = table[i];
                    table[i] = table[ixj];
                    table[ixj] = temp;
                }
            ...
            if ((i & k) != 0) {
                if (compare_rotations(table[i], table[ixj], inputMessage, n)) {
                    int temp = table[i];
                    table[i] = table[ixj];
                    table[ixj] = temp;
                }
